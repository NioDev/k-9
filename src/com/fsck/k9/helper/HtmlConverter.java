...
public class HtmlConverter
{
	...
	{
		return Html.fromHtml(html, null, new HtmlToTextTagHandler()).toString().replace(PREVIEW_OBJECT_CHARACTER, PREVIEW_OBJECT_REPLACEMENT)
	...
	private static class HtmlToTextTagHandler implements Html.TagHandler
	{
		...
		private Object getOpeningAnnotation(Editable output)
		{
			...
			for (int i = objs.length - 1; i >= 0; i--)
			{
				...
				if (output.getSpanFlags(objs[i]) == Spannable.SPAN_MARK_MARK && span.getKey().equals(IGNORED_ANNOTATION_KEY)
	 ...
	 * {@link com.fsck.k9.view#MessageWebView.setText(String)
	...
	public static String textToHtml(String text)
	{
		...
		// blockquote.
		text = text.replaceAll("\\Q" + HTML_NEWLINE + "\\E((\\Q" + HTML_NEWLINE + "\\E)+?)\\Q" + HTML_BLOCKQUOTE_END + "\\E", HTML_BLOCKQUOTE_END + "$1");
	...
	private static void appendchar(StringBuilder buff, int c)
	{
		switch (c)
			...
			case '<':
				...
				break;
			case '>':
				// We use a token here which can't occur in htmlified text
				// because &gt; is valid
				// within links (where > is not), and linkifying links will
				// include it if we
			...
			case '\r':
				break;
			case '\n':
				// pine treats <br> as two newlines, but <br/> as one newline.
				// Use <br/> so our messages aren't
				// doublespaced.
	...
	public static String cssStylePre()
	{
		...
		return "<style type=\"text/css\"> pre." + K9MAIL_CSS_CLASS + " {white-space: pre-wrap; word-wrap:break-word; " + "font-family: " + font
