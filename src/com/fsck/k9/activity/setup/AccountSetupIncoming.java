...
public class AccountSetupIncoming extends K9Activity implements OnClickListener
{
	...
	private static final ConnectionSecurity[] CONNECTION_SECURITY_TYPES =
		{ ConnectionSecurity.NONE, ConnectionSecurity.SSL_TLS_OPTIONAL, ConnectionSecurity.SSL_TLS_REQUIRED, ConnectionSecurity.STARTTLS_OPTIONAL,
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		SpinnerOption authTypeSpinnerOptions[] =
			{ new SpinnerOption(0, AUTH_TYPES[0]), new SpinnerOption(1, AUTH_TYPES[1]) };

		ArrayAdapter<SpinnerOption> securityTypesAdapter = new ArrayAdapter<SpinnerOption>(this, android.R.layout.simple_spinner_item, securityTypes);
		securityTypesAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
		mSecurityTypeView.setAdapter(securityTypesAdapter);

		ArrayAdapter<SpinnerOption> authTypesAdapter = new ArrayAdapter<SpinnerOption>(this, android.R.layout.simple_spinner_item, authTypeSpinnerOptions);
	...
	{
		mNextButton.setEnabled(Utility.requiredFieldValid(mUsernameView) && Utility.requiredFieldValid(mPasswordView) && Utility.domainFieldValid(mServerView)
		...
		if (resultCode == RESULT_OK)
		{
			...
			else
			{
				...
				try
				{
					...
					URI uri = new URI(oldUri.getScheme(), usernameEnc + ":" + passwordEnc, oldUri.getHost(), oldUri.getPort(), null, null, null);
	...
	protected void onNext()
	{
		try
		{
			ConnectionSecurity connectionSecurity = CONNECTION_SECURITY_TYPES[(Integer) ((SpinnerOption) mSecurityTypeView.getSelectedItem()).value];
			...
			ServerSettings settings = new ServerSettings(mStoreType, host, port, connectionSecurity, authType, username, password, extra);
