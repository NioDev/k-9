	...
	public class TracingWakeLock
	{
		...
		public void release()
		{
			if (startTime != null)
			{
				...
				if (K9.DEBUG)
				{
					Log.v(K9.LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + ": releasing after " + (endTime - startTime) + " ms, timeout = "
							+ timeout + " ms");
				}
			}
			else
				...
				synchronized (timer)
				{
					...
					timerTask = new TimerTask()
					{
						...
						public void run()
						{
							...
							{
								Long endTime = System.currentTimeMillis();
								Log.i(K9.LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id + ": has been active for " + (endTime - startTime)
