...
public class SettingsImporter
{
	...
	public static class ImportResults
	{
		...
		private ImportResults(boolean globalSettings, List<AccountDescriptionPair> importedAccounts, List<AccountDescription> errorneousAccounts)
		{
			this.globalSettings = globalSettings;
	 ...
	 *             In case of an error.
	 */
	public static ImportResults importSettings(Context context, InputStream inputStream, boolean globalSettings, List<String> accountUuids, boolean overwrite)
			throws SettingsImportExportException
	{
			...
			if (accountUuids != null && accountUuids.size() > 0)
			{
				if (imported.accounts != null)
				{
					for (String accountUuid : accountUuids)
					{
						if (imported.accounts.containsKey(accountUuid))
						{
							...
							try
							{
								...
								if (editor.commit())
								{
									if (K9.DEBUG)
									{
										Log.v(K9.LOG_TAG, "Committed settings for account \"" + importResult.imported.name + "\" to the settings database.");
									}
								...
								else
								{
									if (K9.DEBUG)
									{
										Log.w(K9.LOG_TAG, "Error while committing settings for " + "account \"" + importResult.original.name
							...
							catch (InvalidSettingValueException e)
							{
								if (K9.DEBUG)
								{
									Log.e(K9.LOG_TAG, "Encountered invalid setting while " + "importing account \"" + account.name + "\"", e);
								}
						...
						{
							Log.w(K9.LOG_TAG, "Was asked to import account with UUID " + accountUuid + ". But this account wasn't found.");
						}
					}
	...
	private static void importGlobalSettings(SharedPreferences storage, SharedPreferences.Editor editor, int contentVersion, ImportedSettings settings)
	{
		...
		// import file.
		Map<String, String> mergedSettings = new HashMap<String, String>(GlobalSettings.getGlobalSettings(storage));
	...
	private static AccountDescriptionPair importAccount(Context context, SharedPreferences.Editor editor, int contentVersion, ImportedAccount account,
			boolean overwrite) throws InvalidSettingValueException
	{
		...
		// password
		boolean createAccountDisabled = (incoming.password == null || incoming.password.length() == 0);
		...
		{
			writeSettings = new HashMap<String, String>(AccountSettings.getAccountSettings(prefs.getPreferences(), uuid));
	...
	private static void importFolder(SharedPreferences.Editor editor, int contentVersion, String uuid, ImportedFolder folder, boolean overwrite,
			Preferences prefs)
	{
	...
	private static void importIdentities(SharedPreferences.Editor editor, int contentVersion, String uuid, ImportedAccount account, boolean overwrite,
			Account existingAccount, Preferences prefs) throws InvalidSettingValueException
	{
		...
		for (ImportedIdentity identity : account.identities)
		{
			...
			putString(editor, accountKeyPrefix + Account.IDENTITY_DESCRIPTION_KEY + identitySuffix, identityDescription);
			...
			if (identity.settings != null)
			{
				...
				if (mergeSettings)
				{
					writeSettings = new HashMap<String, String>(IdentitySettings.getIdentitySettings(prefs.getPreferences(), uuid, writeIdentityIndex));
					writeSettings.putAll(stringSettings);
				}
				else
				{
	...
	private static Imported parseSettings(InputStream inputStream, boolean globalSettings, List<String> accountUuids, boolean overview)
			throws SettingsImportExportException
	{
	...
	private static Imported parseRoot(XmlPullParser xpp, boolean globalSettings, List<String> accountUuids, boolean overview) throws XmlPullParserException,
			IOException, SettingsImportExportException
	{
	...
	private static Map<String, ImportedAccount> parseAccounts(XmlPullParser xpp, List<String> accountUuids, boolean overview) throws XmlPullParserException,
			IOException
	{
	...
	private static class ImportedServerSettings extends ServerSettings
	{
		...
		{
			super(server.type, server.host, convertPort(server.port), convertConnectionSecurity(server.connectionSecurity), server.authenticationType,
		...
		{
			return (mImportedServer.extras != null) ? Collections.unmodifiableMap(mImportedServer.extras.settings) : null;
		}
