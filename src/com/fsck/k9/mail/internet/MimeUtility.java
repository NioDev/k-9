...
public class MimeUtility
{
	...
	private static final String[][] MIME_TYPE_REPLACEMENT_MAP = new String[][]
		{
			...
			{ "image/pjpeg", "image/jpeg" }, // see
												// issue
												// 1712
					{ "application/x-zip-compressed", "application/zip" } // see
																			// issue
																			// 3791
		};
	 ...
	 */
	public static Body decodeBody(InputStream in, String contentTransferEncoding, String contentType) throws IOException, MessagingException
	{
		/*
	 ...
	 *             In case of an error.
	 */
	private static List<Viewable> findHtmlPart(Multipart multipart, Set<Part> knownTextParts, List<Part> attachments, boolean directChild)
			throws MessagingException
	{
		List<Viewable> viewables = new ArrayList<Viewable>();
		...
		for (int i = 0; i < childCount; i++)
		{
			...
			else if (!(directChild && partFound) && isPartTextualBody(part) && part.getMimeType().equalsIgnoreCase("text/html"))
			{
				Html html = new Html(part);
				viewables.add(html);
				partFound = true;
	 ...
	 *             In case of an error.
	 */
	private static void addMessageHeaderText(Context context, StringBuilder text, Message message) throws MessagingException
	{
		// From: <sender>
	 ...
	 *             In case of an error.
	 */
	private static void addMessageHeaderHtml(Context context, StringBuilder html, Message message) throws MessagingException
	{
		...
		// Subject: <subject>
		String subject = message.getSubject();
		addTableRow(html, context.getString(R.string.message_compose_quote_header_subject), (subject == null) ? context.getString(R.string.general_no_subject)
				: subject);
	...
	private static String getJisVariantFromAddress(String address)
	{
		if (address == null)
			return null;
		if (isInDomain(address, "docomo.ne.jp") || isInDomain(address, "dwmail.jp") || isInDomain(address, "pdx.ne.jp") || isInDomain(address, "willcom.com")
				|| isInDomain(address, "emnet.ne.jp") || isInDomain(address, "emobile.ne.jp"))
			return "docomo";
		else if (isInDomain(address, "softbank.ne.jp") || isInDomain(address, "vodafone.ne.jp") || isInDomain(address, "disney.ne.jp")
	...
	public static String readToString(InputStream in, String charset) throws IOException
	{
		...
		if (charset.length() > 17 && charset.startsWith("x-") && charset.endsWith("-shift_jis-2007") && !Charset.isSupported(charset))
		{
		...
		for (String[] rule : CHARSET_FALLBACK_MAP)
		{
			...
			{
				Log.e(K9.LOG_TAG, "I don't know how to deal with the charset " + charset + ". Falling back to " + rule[1]);
				charset = rule[1];
				try
				{
					supported = Charset.isSupported(charset);
