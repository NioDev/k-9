...
public class ImapStore extends Store
{
	...
	private static int MAX_DELAY_TIME = 5 * 60 * 1000; // 5
														// minutes
	...
	public static ImapStoreSettings decodeUri(String uri)
	{
		...
		return new ImapStoreSettings(host, port, connectionSecurity, authenticationType, username, password, autoDetectNamespace, pathPrefix);
	}
	...
	public static class ImapStoreSettings extends ServerSettings
	{
		...
		protected ImapStoreSettings(String host, int port, ConnectionSecurity connectionSecurity, String authenticationType, String username, String password,
		...
		{
			return new ImapStoreSettings(host, port, connectionSecurity, authenticationType, username, newPassword, autoDetectNamespace, pathPrefix);
		}
	}
	...
	private List<? extends Folder> listFolders(ImapConnection connection, boolean LSUB) throws IOException, MessagingException
	{
		...
		List<ImapResponse> responses = connection.executeSimpleCommand(String.format("%s \"\" %s", commandResponse, encodeString(getCombinedPrefix() + "*")));
		...
		for (ImapResponse response : responses)
		{
			if (ImapResponseParser.equalsIgnoreCase(response.get(0), commandResponse))
			{
				...
				{
					Log.w(K9.LOG_TAG, "Skipping incorrectly parsed " + commandResponse + " reply: " + response);
					continue;
				}
				...
				{
					Log.w(K9.LOG_TAG, "Folder name not correctly encoded with the UTF-7 variant " + "as defined by RFC 3501: " + response.getString(3), e);
	 ...
	 * {@link #listFolders(com.fsck.k9.mail.store.ImapStore.ImapConnection, boolean)}
	...
	private void autoconfigureFolders(final ImapConnection connection) throws IOException, MessagingException
	{
		...
		final List<ImapResponse> responses = connection.executeSimpleCommand(String.format("%s%s \"\" %s", commandResponse, commandOptions,
...
<<<<<<< HEAD
	...
	class ImapFolder extends Folder
	{
		...
		protected List<ImapResponse> executeSimpleCommand(String command, boolean sensitve, UntaggedHandler untaggedHandler) throws MessagingException,
				IOException
		{
		...
		public List<ImapResponse> internalOpen(int mode) throws MessagingException
		{
			...
			{
				msgSeqUidMap.clear();
				String command = String.format("%s %s", mode == OPEN_MODE_RW ? "SELECT" : "EXAMINE", encodeString(encodeFolderName(getPrefixedName())));
		...
		public boolean exists() throws MessagingException
		{
			...
			try
			{
				connection.executeSimpleCommand(String.format("STATUS %s (UIDVALIDITY)", encodeString(encodeFolderName(getPrefixedName()))));
				mExists = true;
				return true;
			}
			catch (ImapException ie)
		...
		public boolean create(FolderType type) throws MessagingException
		{
			...
			try
			{
				connection.executeSimpleCommand(String.format("CREATE %s", encodeString(encodeFolderName(getPrefixedName()))));
				return true;
			}
			catch (ImapException ie)
			{
		...
		public Map<String, String> copyMessages(Message[] messages, Folder folder) throws MessagingException
		{
			...
			try
			{
				...
				if (!exists(remoteDestName))
				{
					...
					if (K9.DEBUG)
					{
						Log.i(K9.LOG_TAG, "ImapFolder.copyMessages: attempting to create remote " + "folder '" + remoteDestName + "' for " + getLogId());
					}
				...
				List<ImapResponse> responses = executeSimpleCommand(String.format("UID COPY %s %s", Utility.combine(uids, ','), remoteDestName));
		...
		public void delete(Message[] messages, String trashFolderName) throws MessagingException
		{
			...
			else
			{
				...
				{
					throw new MessagingException("IMAPMessage.delete: remote Trash folder " + trashFolderName + " does not exist and could not be created for "
							+ getLogId(), true);
				}
			}
		}
				...
				throws MessagingException
		{
			ImapSearcher searcher = new ImapSearcher()
			{
				public List<ImapResponse> search() throws IOException, MessagingException
				{
					return executeSimpleCommand(String
							.format("UID SEARCH %s%s", Utility.combine(mesgSeqs.toArray(), ','), includeDeleted ? "" : " NOT DELETED"));
				}
			};
			return search(searcher, listener).toArray(EMPTY_MESSAGE_ARRAY);
		}
				...
				throws MessagingException
		{
			ImapSearcher searcher = new ImapSearcher()
			{
				public List<ImapResponse> search() throws IOException, MessagingException
				{
					return executeSimpleCommand(String.format("UID SEARCH UID %s%s", Utility.combine(mesgUids.toArray(), ','), includeDeleted ? ""
							: " NOT DELETED"));
				}
			};
			return search(searcher, listener).toArray(EMPTY_MESSAGE_ARRAY);
			...
			for (int windowStart = 0; windowStart < messages.length; windowStart += (FETCH_WINDOW_SIZE))
			{
				...
				try
				{
					mConnection.sendCommand(
							String.format("UID FETCH %s (%s)", Utility.combine(uidWindow.toArray(new String[uidWindow.size()]), ','),
						...
						if (response.mTag == null && ImapResponseParser.equalsIgnoreCase(response.get(1), "FETCH"))
						{
							...
							if (uid != null)
							{
								try
								{
									...
									if (K9.DEBUG)
									{
										Log.v(K9.LOG_TAG, "Stored uid '" + uid + "' for msgSeq " + msgSeq + " into map " /*
																														 * +
																														 * msgSeqUidMap
																														 * .
																														 * toString
		...
		public void fetchPart(Message message, Part part, MessageRetrievalListener listener) throws MessagingException
		{
			...
			{
				fetch = String.format(Locale.US, "BODY.PEEK[TEXT]<0.%d>", mAccount.getMaximumAutoDownloadMessageSize());
			}
			else
			{
					...
					if ((response.mTag == null) && (ImapResponseParser.equalsIgnoreCase(response.get(1), "FETCH")))
					{
						...
						if (literal != null)
						{
							...
							else if (literal instanceof String)
							{
								...
								part.setBody(MimeUtility.decodeBody(bodyStream, contentTransferEncoding, contentType));
							}
							else
							{
								// This shouldn't happen
		...
		private void parseBodyStructure(ImapList bs, Part part, String id) throws MessagingException
		{
			...
			else
			{
				...
				if (bodyParams != null)
				{
					...
					{
						contentType.append(String.format(";\r\n %s=\"%s\"", bodyParams.getString(i), bodyParams.getString(i + 1)));
					}
				}
				...
				if (bodyDisposition != null && !bodyDisposition.isEmpty())
				{
					...
					if ((bodyDisposition.size() > 1) && (bodyDisposition.get(1) instanceof ImapList))
					{
						...
						{
							contentDisposition.append(String.format(";\r\n %s=\"%s\"", bodyDispositionParams.getString(i).toLowerCase(Locale.US),
		...
		public Map<String, String> appendMessages(Message[] messages) throws MessagingException
		{
			...
			try
			{
				...
				{
					mConnection.sendCommand(
							String.format("APPEND %s (%s) {%d}", encodeString(encodeFolderName(getPrefixedName())), combineFlags(message.getFlags()),
		...
		public String getUidFromMessageId(Message message) throws MessagingException
		{
			try
			{
				...
				List<ImapResponse> responses = executeSimpleCommand(String.format("UID SEARCH HEADER MESSAGE-ID %s", encodeString(messageId)));
				for (ImapResponse response1 : responses)
				{
					if (response1.mTag == null && ImapResponseParser.equalsIgnoreCase(response1.get(0), "SEARCH") && response1.size() > 1)
					{
		...
		private String combineFlags(Flag[] flags)
		{
			...
			for (Flag flag : flags)
			{
				...
				else if (flag == Flag.FORWARDED && (mCanCreateKeywords || mPermanentFlagsIndex.contains(Flag.FORWARDED)))
				{
		...
		public void setFlags(Flag[] flags, boolean value) throws MessagingException
		{
			...
			try
			{
				executeSimpleCommand(String.format("UID STORE 1:* %sFLAGS.SILENT (%s)", value ? "+" : "-", combineFlags(flags)));
			}
			catch (IOException ioe)
			{
				throw ioExceptionHandler(mConnection, ioe);
		...
		public void setFlags(Message[] messages, Flag[] flags, boolean value) throws MessagingException
		{
			...
			try
			{
				executeSimpleCommand(String.format("UID STORE %s %sFLAGS.SILENT (%s)", Utility.combine(uids, ','), value ? "+" : "-", combineFlags(flags)));
			}
			catch (IOException ioe)
			{
				throw ioExceptionHandler(mConnection, ioe);
		 ...
		 */
		@Override
		public List<Message> search(final String queryString, final Flag[] requiredFlags, final Flag[] forbiddenFlags) throws MessagingException
		{
	...
	public static class ImapConnection
	{
		...
		private List<ImapResponse> receiveCapabilities(List<ImapResponse> responses)
		{
			for (ImapResponse response : responses)
			{
				...
				if (capabilityList != null && !capabilityList.isEmpty() && ImapResponseParser.equalsIgnoreCase(capabilityList.get(0), CAPABILITY_CAPABILITY))
				{
					if (K9.DEBUG)
					{
			...
			try
			{
				...
				for (int i = 0; i < addresses.length; i++)
				{
					try
					{
						...
						if (connectionSecurity == CONNECTION_SECURITY_SSL_REQUIRED || connectionSecurity == CONNECTION_SECURITY_SSL_OPTIONAL)
						{
							SSLContext sslContext = SSLContext.getInstance("TLS");
							boolean secure = connectionSecurity == CONNECTION_SECURITY_SSL_REQUIRED;
							sslContext.init(null, new TrustManager[]
								{ TrustManagerFactory.get(mSettings.getHost(), secure) }, new SecureRandom());
						...
						|| mSettings.getConnectionSecurity() == CONNECTION_SECURITY_TLS_REQUIRED)
				{
					...
					if (hasCapability("STARTTLS"))
					{
						...
						sslContext.init(null, new TrustManager[]
							{ TrustManagerFactory.get(mSettings.getHost(), secure) }, new SecureRandom());
						mSocket = sslContext.getSocketFactory().createSocket(mSocket, mSettings.getHost(), mSettings.getPort(), true);
				...
				try
				{
					...
					else if (mSettings.getAuthType() == AuthType.PLAIN)
					{
						receiveCapabilities(executeSimpleCommand(
								String.format("LOGIN %s %s", ImapStore.encodeString(mSettings.getUsername()), ImapStore.encodeString(mSettings.getPassword())),
								true));
					}
					authSuccess = true;
				}
				...
				if (K9.DEBUG)
					Log.d(K9.LOG_TAG, "NAMESPACE = " + hasCapability(CAPABILITY_NAMESPACE) + ", mPathPrefix = " + mSettings.getPathPrefix());
		...
		protected void authCramMD5() throws AuthenticationFailedException, MessagingException
		{
			try
			{
				...
				byte[] b64CRAM = Authentication.computeCramMd5Bytes(mSettings.getUsername(), mSettings.getPassword(), b64NonceTrim);
		...
		public List<ImapResponse> executeSimpleCommand(String command, boolean sensitive, UntaggedHandler untaggedHandler) throws IOException, ImapException,
				MessagingException
		{
				...
				if (response.mTag != null && !response.mTag.equalsIgnoreCase(tag))
				{
					...
					while (iter.hasNext())
					{
						...
						if (delResponse.mTag != null
								|| delResponse.size() < 2
								|| (!ImapResponseParser.equalsIgnoreCase(delResponse.get(1), "EXISTS") && !ImapResponseParser.equalsIgnoreCase(
										delResponse.get(1), "EXPUNGE")))
						{
							iter.remove();
						}
					}
	...
	public class ImapFolderPusher extends ImapFolder implements UntaggedHandler
	{
						...
						try
						{
							...
							if (newUidNext == -1)
							{
								...
								if (highestUid != -1L)
								{
									...
									if (K9.DEBUG)
										Log.d(K9.LOG_TAG, "highest UID = " + highestUid + ", set newUidNext to " + newUidNext);
								}
							}
							...
							else
							{
								...
								{
									if (K9.DEBUG)
										Log.i(K9.LOG_TAG, "Processing " + storedUntaggedResponses.size() + " untagged responses from previous commands for "
												+ getLogId());
		...
		protected void handleUntaggedResponse(ImapResponse response)
		{
			if (response.mTag == null && response.size() > 1)
			{
				...
				if (ImapResponseParser.equalsIgnoreCase(responseType, "FETCH") || ImapResponseParser.equalsIgnoreCase(responseType, "EXPUNGE")
	...
	private static class FetchPartCallback implements ImapResponseParser.IImapResponseCallback
	{
		...
		public Object foundLiteral(ImapResponse response, FixedLengthInputStream literal) throws IOException, Exception
		{
			if (response.mTag == null && ImapResponseParser.equalsIgnoreCase(response.get(1), "FETCH"))
			{
				...
				return MimeUtility.decodeBody(literal, contentTransferEncoding, contentType);
			}
			return null;
		}
	}
