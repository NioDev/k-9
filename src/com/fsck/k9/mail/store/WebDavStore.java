...
public class WebDavStore extends Store
{
	...
	public static WebDavStoreSettings decodeUri(String uri)
	{
		...
		return new WebDavStoreSettings(host, port, connectionSecurity, null, username, password, alias, path, authPath, mailboxPath);
	}
	...
	public static class WebDavStoreSettings extends ServerSettings
	{
		...
		protected WebDavStoreSettings(String host, int port, ConnectionSecurity connectionSecurity, String authenticationType, String username,
				String password, String alias, String path, String authPath, String mailboxPath)
		{
		...
		{
			return new WebDavStoreSettings(host, port, connectionSecurity, authenticationType, username, newPassword, alias, path, authPath, mailboxPath);
		}
	}

	private short mConnectionSecurity;
	private String mUsername; /*
							 * Stores the username for authentications
							 */
	private String mAlias; /*
							 * Stores the alias for the user 's mailbox
							 */
	private String mPassword; /*
							 * Stores the password for authentications
							 */
	private String mUrl; /*
						 * Stores the base URL for the server
						 */
	private String mHost; /*
						 * Stores the host name for the server
						 */
	private int mPort;
	...
	{
		String root;
		if (mConnectionSecurity == CONNECTION_SECURITY_TLS_REQUIRED || mConnectionSecurity == CONNECTION_SECURITY_SSL_REQUIRED
				|| mConnectionSecurity == CONNECTION_SECURITY_TLS_OPTIONAL || mConnectionSecurity == CONNECTION_SECURITY_SSL_OPTIONAL)
		{
			root = "https";
		}
		else
		...
		try
		{
			if (mAuthentication == AUTH_TYPE_NONE)
			{
				...
				if (info.requiredAuthType == AUTH_TYPE_BASIC)
				{
					...
					{
						throw new MessagingException("Error with code " + response.getStatusLine().getStatusCode() + " during request processing: "
	...
	private boolean testAuthenticationResponse(HttpResponse response) throws MessagingException
	{
		...
		if (((statusCode >= 200 && statusCode < 300) || statusCode == 302) && mAuthCookies != null && !mAuthCookies.getCookies().isEmpty())
		{
	 ...
	 * the response.
	 */
	private DataSet processRequest(String url, String method, String messageBody, HashMap<String, String> headers) throws MessagingException
	{
	...
	class WebDavFolder extends Folder
	{
		...
		private void fetchMessages(Message[] messages, MessageRetrievalListener listener, int lines) throws MessagingException
		{
			...
			for (int i = 0, count = messages.length; i < count; i++)
			{
				...
				{
					wdMessage.setUrl(getMessageUrls(new String[]
						{ wdMessage.getUid() }).get(wdMessage.getUid()));
		...
		public Message[] appendWebDavMessages(Message[] messages) throws MessagingException
		{
			...
			for (Message message : messages)
			{
				...
				try
				{
					...
					if (statusCode < 200 || statusCode > 300)
					{
						throw new IOException("Error with status code " + statusCode + " while sending/appending message.  Response = "
	...
	class WebDavMessage extends MimeMessage
	{
		...
		public void delete(String trashFolderName) throws MessagingException
		{
			...
			wdFolder.moveMessages(new Message[]
				{ this }, wdFolder.getStore().getFolder(trashFolderName));
		}
	...
	public class HttpGeneric extends HttpEntityEnclosingRequestBase
	{
		...
		public HttpGeneric(final String uri)
		{
			...
			if (K9.DEBUG && K9.DEBUG_PROTOCOL_WEBDAV)
			{
				Log.v(K9.LOG_TAG, "url = '" + url + "' length = " + url.length() + ", end = '" + end + "' length = " + end.length());
			}
			url = url + "/" + end;
