...
public class MimeMessage extends Message
{
	...
	public String getMessageId() throws MessagingException
	{
		...
		if (mMessageId == null)
		{ // even after checking the header
	...
	public void setReferences(String references) throws MessagingException
	{
		...
		if (originalLength >= limit)
		{
			...
			final String firstReference = references.substring(start, references.indexOf('<', start + 1));

			// Find longest tail
			final String tail = references.substring(references.indexOf('<', firstReference.length() + originalLength - limit));
	...
	class MimeMessageBuilder implements ContentHandler
	{
		...
		private void expect(Class<?> c)
		{
			...
			{
				throw new IllegalStateException("Internal stack error: " + "Expected '" + c.getName() + "' found '" + stack.peek().getClass().getName() + "'");
			}
		}
		...
		public void body(BodyDescriptor bd, InputStream in) throws IOException
		{
			...
			try
			{
				Body body = MimeUtility.decodeBody(in, bd.getTransferEncoding(), bd.getMimeType());
	...
	public void setUsing7bitTransport() throws MessagingException
	{
		 ...
		 */
		if (mBody instanceof CompositeBody && !"multipart/signed".equalsIgnoreCase(type))
		{
			setEncoding(MimeUtil.ENC_7BIT);
			// recurse
			((CompositeBody) mBody).setUsing7bitTransport();
		}
		else if (!MimeUtil.ENC_8BIT.equalsIgnoreCase(getFirstHeader(MimeHeader.HEADER_CONTENT_TRANSFER_ENCODING)))
		{
			return;
		}
		else if (type != null && (type.equalsIgnoreCase("multipart/signed") || type.toLowerCase(Locale.US).startsWith("message/")))
		{
			/*
